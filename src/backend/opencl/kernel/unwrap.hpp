...
namespace opencl
{
    namespace kernel
    {
        ...
        template<typename T>
        void unwrap_dilated(Param out, const Param in,
                       const dim_t wx, const dim_t wy,
                       const dim_t sx, const dim_t sy,
                       const dim_t px, const dim_t py,
                       ...
                       const dim_t nx, const bool is_column)
        {
            ...
            if (entry.prog==0 && entry.ker==0) {
                ...
                Program prog;
                buildProgram(prog, unwrap_dilated_cl, unwrap_dilated_cl_len, options.str());
            ...
            unwrapOp(EnqueueArgs(getQueue(), global, local),
                    *out.data, out.info, *in.data, in.info, wx, wy, sx, sy, px, py, dx, dy, nx, reps);
