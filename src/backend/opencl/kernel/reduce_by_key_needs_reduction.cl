...
__kernel
void test_needs_reduction(__global int *needs_another_reduction,
                          ...
                          KParam iKInfo, int n)
{
    ...
    int update_key = (lid < DIMX - 2) && (k == keys[lid + 1]) && (gid < (n-1));
    __local int remaining_updates;
    atomic_or(&remaining_updates, update_key);

    barrier(CLK_LOCAL_MEM_FENCE);

    //TODO: single per warp? change to assignment rather than atomicOr
    if(remaining_updates)
        atomic_or(needs_another_reduction, remaining_updates);
    ...
    if(gid == ((bid + 1) * DIMX) - 1 && bid < get_num_groups(0) - 1) {
        ...
        int k1 = iKeys[gid + 1];
        if(k0 == k1) {
            atomic_or(needs_block_boundary_reduced, 1);
        }
    }
}
