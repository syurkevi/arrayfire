...
#undef INSTANTIATE
                            ...
                            const dim_t dx, const dim_t dy, const bool is_column)
    {
        ...
        Array<T> outArray = createEmptyArray<T>(odims);
        kernel::unwrap_dilated<T>(outArray, in, wx, wy, sx, sy, px, py, dx, dy, nx, is_column);
...
#define INSTANTIATE(T)                                                                              \
    template Array<T> unwrap_dilated<T> (const Array<T> &in, const dim_t wx, const dim_t wy,        \
                    const dim_t sx, const dim_t sy, const dim_t px, const dim_t py, const dim_t dx, \
