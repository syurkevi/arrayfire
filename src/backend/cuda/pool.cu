                         ...
                         af_pooling_type pool_type)
{
    ...
    CUDNN(cudnnDestroyTensorDescriptor(input_descriptor));
    CUDNN(cudnnDestroyTensorDescriptor(output_descriptor));
    CUDNN(cudnnDestroyTensorDescriptor(grad_descriptor));
    CUDNN(cudnnDestroyTensorDescriptor(ingrad_descriptor));
...
{
    if(supported_cudnn_type<T>())
        return pool2Grad_cudnn(incoming_gradient, original_input, pooled_output, pool_width, pool_height, padding_width, padding_height, stride_width, stride_height, pool_type);
    else
        return pool2Grad_native(incoming_gradient, original_input, pooled_output, pool_width, pool_height, padding_width, padding_height, stride_width, stride_height, pool_type);
}
