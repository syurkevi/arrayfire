...
void convolve2_direct(Param<InT> out, CParam<InT> signal,
                      CParam<AccT> filter, const af::dim4 stride,
                      const af::dim4 padding, const af::dim4 dilation)
{
    dim_t flen = (dim_t)filter.dims().elements();
    ...
    auto oStrides = out.strides();
    auto sStrides = signal.strides();
    auto fStrides = filter.strides();

    bool isConv = true;

    // Number of pixels in output
    int nOutPixels = oDims[0] * oDims[1];

    // Number of pixels in filter
    int nFilterPixels = fDims[0] * fDims[1];

    const int half_fw = (fDims[0] / 2) * dilation[0];
    const int half_fh = (fDims[1] / 2) * dilation[1];

    // For each image in the output
    for(int n=0; n < oDims[3]; ++n) {
        // For each feature layer of the output
        for(int k=0; k < oDims[2]; ++k) {
            const int imgOffset = n*oStrides[3] + k*oStrides[2];
            InT *optr = out.get() + imgOffset;
            InT const * const sptr = signal.get() + n*sStrides[3] + k*sStrides[2];
            AccT const * const fptr = filter.get() + n*fStrides[3] + k*fStrides[2];
            //InT *tptr = temp.get() + b2*tStrides[2] + t_b3Off;

            for(int w=0; w < oDims[1]; ++w) {
                for(int h=0; h < oDims[0]; ++h) {

                    InT tmp = scalar<AccT>(0);
                    for(int fx=0; fx < fDims[1]; fx+=dilation[1]) {
                        for(int fy=0; fy < fDims[0]; fy+=dilation[0]) {
                            const bool inImg = (w - half_fw + fx >= 0 && w - half_fw + fx < oDims[1] &&
                                                h - half_fh + fy >= 0 && h - half_fh + fy < oDims[0]);
                            if(inImg) {
                                tmp = sptr[w*sStrides[1] + h*sStrides[0]] * fptr[fx*fStrides[1] + fy*fStrides[0]] + tmp;
                            }
                        }
                    }
                    optr[w*oStrides[1] + h*oStrides[0]] = tmp;
                }
            }
        }
    }
