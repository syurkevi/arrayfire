...
class Conv2ConsistencyTest  : public ::testing::TestWithParam<conv2_strided_params> { };

conv2_strided_params conv2_consistency_data(dim4 signal_sz, dim4 filt_sz) {
    dim4 stride(1, 1);
    dim4 padding(filt_sz[0] / 2, filt_sz[1] / 2);
    dim4 dilation(1, 1);
    std::string testname = "conv2_consistency_" +
              std::to_string(signal_sz[0]) + std::to_string(signal_sz[1]) + std::to_string(signal_sz[2]) + std::to_string(signal_sz[3]) + "__" +
              std::to_string(filt_sz[0]) + std::to_string(filt_sz[1]) + std::to_string(filt_sz[2]) + std::to_string(filt_sz[3]) + "__" +
             "s" + std::to_string(stride[0]) + std::to_string(stride[1]) + "_" +
             "p" +  std::to_string(padding[0]) + std::to_string(padding[1]) + "_" +
             "d" +  std::to_string(dilation[0]) + std::to_string(dilation[1]);

    return conv2_strided_params(testname, signal_sz, filt_sz, stride, padding, dilation);
}
...
INSTANTIATE_TEST_CASE_P(Conv2Consistency,
                        Conv2ConsistencyTest,
...
TEST_P(Conv2ConsistencyTest, RandomConvolutions) {
    ...
    array out = convolve2(signal, filter, params.stride_, params.padding_, params.dilation_);
...
template<typename T>
void convolve2stridedTest(string pTestFile, dim4 stride, dim4 padding, dim4 dilation)
{
    if (noDoubleTests<T>()) return;
    ...
    ASSERT_SUCCESS(af_create_array(&signal, &(in[0].front()),
                sDims.ndims(), sDims.get(), (af_dtype)dtype_traits<float>::af_type));
    ASSERT_SUCCESS(af_create_array(&filter, &(in[1].front()),
                fDims.ndims(), fDims.get(), (af_dtype)dtype_traits<float>::af_type));

    ASSERT_SUCCESS(af_cast(&signal, signal, (af_dtype)dtype_traits<T>::af_type));

    ASSERT_SUCCESS(af_convolve2_strided(&convolved, signal, filter,
    ...
    for(auto t : tests[0]) {
        currGoldBar.push_back((T)t);
    }

    size_t nElems = currGoldBar.size();

    dim_t expectedDim0 = 1 + (sDims[0] + 2 * padding[0] - (((fDims[0] - 1) * dilation[0]) + 1)) / stride[0];
    dim_t expectedDim1 = 1 + (sDims[1] + 2 * padding[1] - (((fDims[1] - 1) * dilation[1]) + 1)) / stride[1];

    ASSERT_VEC_ARRAY_NEAR(currGoldBar, dim4(expectedDim0, expectedDim1, fDims[3], sDims[3]), convolved, 1e-4);
...
template<typename T>
void convolve2GradientTest(string pTestFile, dim4 stride, dim4 padding, dim4 dilation)
{
    if (noDoubleTests<T>()) return;
    ...
    ASSERT_SUCCESS(af_create_array(&convolved, &(currGoldBar.front()),
                cDims.ndims(), cDims.get(), (af_dtype)dtype_traits<T>::af_type));

    af_array incoming_gradient = 0;
    ASSERT_SUCCESS(af_constant(&incoming_gradient, 1, cDims.ndims(), cDims.get(), (af_dtype)dtype_traits<T>::af_type));

    af_array filter_gradient = 0;
    ASSERT_SUCCESS(af_convolve2Gradient(&filter_gradient, incoming_gradient,
                                        signal, filter,
                                        convolved,
                                        stride.ndims(),   stride.get(),
                                        padding.ndims(),  padding.get(),
    ...
    ASSERT_SUCCESS(af_convolve2Gradient(&data_gradient, incoming_gradient,
                                        signal, filter,
                                        convolved,
                                        stride.ndims(),   stride.get(),
                                        padding.ndims(),  padding.get(),
...
// create a list of types to be tested
typedef ::testing::Types<float, double> TestTypesStrided; //TODO: integral types??
...
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig810_filt33_s11_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig81011_filt3311_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt33_s11_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt33_s33_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s33_p11_d11.test"), dim4(3,3), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt33_s33_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s33_p11_d11.test"), dim4(3,3), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt55_s55_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt5511_s55_p11_d11.test"), dim4(5,5), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt55_s55_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt5511_s55_p11_d11.test"), dim4(5,5), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt77_s77_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt7711_s77_p11_d11.test"), dim4(7,7), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt77_s77_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt7711_s77_p11_d11.test"), dim4(7,7), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt33_s11_p11_d22)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s11_p11_d22.test"), dim4(1,1), dim4(1,1), dim4(2,2));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt33_s11_p11_d22)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s11_p11_d22.test"), dim4(1,1), dim4(1,1), dim4(2,2));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt33_s11_p11_d33)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s11_p11_d33.test"), dim4(1,1), dim4(1,1), dim4(3,3));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt33_s11_p11_d33)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3311_s11_p11_d33.test"), dim4(1,1), dim4(1,1), dim4(3,3));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt35_s11_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3511_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt35_s11_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3511_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt53_s11_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt5311_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt53_s11_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt5311_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig1010_filt35_s31_p11_d21)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3511_s31_p11_d21.test"), dim4(3,1), dim4(1,1), dim4(2,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig1010_filt35_s31_p11_d21)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig101011_filt3511_s31_p11_d21.test"), dim4(3,1), dim4(1,1), dim4(2,1));
}

TYPED_TEST(ConvolveStrided, Strided_sig81032_filt3334_s11_p11_d11)
{
    convolve2stridedTest<TypeParam>(string(TEST_DIR"/convolve/sig81032_filt3334_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}

TYPED_TEST(ConvolveStrided, Gradient_sig81032_filt3334_s11_p11_d11)
{
    convolve2GradientTest<TypeParam>(string(TEST_DIR"/convolve/sig81032_filt3334_s11_p11_d11.test"), dim4(1,1), dim4(1,1), dim4(1,1));
}
